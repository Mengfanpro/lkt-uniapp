        ...
        <div class='mask' v-if='mask_display'>
            <div class="mask_d" :class='mask_display1?"goodAnimate1":(mask_display?"goodAnimate":"")'>
                ...
                <div style="height: 98rpx;">
                    <div class='mask_quren' @tap="_confirm">{{language.groupDetailed.confirm}}</div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        onShareAppMessage: function (res) {
            let url = '/pagesA/group/groupDetailed?pro_id=' + this.pro_id + '&activity_no=' + this.activity_no + '&isfx=true&fatherId=' + uni.getStorageSync('user_id')
            if (this.a_type == 1) {
                url += '&a_type=1'
            }
        ...
        onLoad (option) {
            ...
            me.LaiKeTuiCommon.getLKTApiUrl().then(() => {
                ...
                let shareHref = uni.getStorageSync('h5_url') + 'pages/index/share?pages=groupDetailed&isfx=true&pro_id=' + this.pro_id +
                    '&activity_no=' + this.activity_no +  '&fatherId=' + uni.getStorageSync('user_id')
                if (this.a_type == 1) {
                    shareHref += '&a_type=1'
                }
        ...
        methods: {
            ...
            showShareMask () {
                var me = this
                this.isLogin(()=>{
                    let shareHref = uni.getStorageSync('h5_url') + 'pages/index/share?pages=groupDetailed&isfx=true&pro_id=' + me.pro_id +
                        '&activity_no=' + me.activity_no  + '&isfx=true&fatherId=' + uni.getStorageSync('user_id');
                    if (this.a_type == 1) {
                        shareHref += '&a_type=1' 
                    }
                    me.shareHref = shareHref;
