<template>
    <view class='page_container' :style="{background:!list.length?'#ffffff':''}">
        ...
        <heads :title='language.seckill.seckill_my.title' :returnR="8"></heads>
        ...
        <ul class="order_header">
            ...
            >
                {{ item }}
                <span class="bottom-hr" v-show="current==index"></span>
            </li>
        </ul>
            ...
            <view v-if='list.length>0' class='page_content'>
                <view v-for="(item,index) in list" :key='index'>
                    ...
                    <view class='list_center' @tap="_goorder(item.id)" v-if="item.list.length">
                        ...
                        <view class='money'>
                            <text>￥{{item.z_price.toFixed(2)}}</text>
                            <text class='size'>x{{item.sum}}</text>
                            <div class="retreat commodity-price-btn" v-if="(order_type == 'evaluete' || order_type == '') && item.list[0].comments_type == 1" @click.stop="comment(item.list[0])">{{language.order.order.button[2]}}</div>
                            <div class="retreat commodity-price-btn" v-if="(order_type == 'evaluete' || order_type == '') && item.list[0].comments_type == 2" @click.stop="comment(item.list[0])">{{language.order.order.button[3]}}</div>
                        </view>
                    </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //立即付款
            pay(id){
                var path = '/pages/order/order_payment?order_id=' + id + '&showPay=true' + '&type=MS';
                // #ifdef H5
                path = '/pages/order/order_payment?order_id=' + id + '&showPay=true&_store=h5' + '&type=MS';
                // #endif
                uni.navigateTo({
                    url: path
                });
            ...
            _goorder (order_id) {
                uni.navigateTo({
                    url: '/pages/order/order?order_id=' + order_id + '&returnR=11' + '&types=MS' + '&order_type=' + this.order_type
                })
                return false
            },
            _toHome () {
