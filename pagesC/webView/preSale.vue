<template>
    <view >  
       <!--  <heads :title="language.goods.goodsDet.Pre_sale_goods" ></heads> -->
        <template>
                ...
                if (res.code == 200) {
                    ...
                    if (options.status || options.id) {
                        if (options.status == 1 || options.status == 0) {
                            this.url = this.url + '#/pagesC/preSale/myPreSale?status=' + options.status + '&access_id=' + this.access_id  + '&phone=' + this.user_phone +'&url=' + this.LaiKeTuiCommon.LKT_H5_DEFURL
                        } else {
                            this.url = this.url + '#/pagesC/preSale/goodsDetailed?pro_id=' + options.id + '&access_id=' + this.access_id  + '&phone=' + this.user_phone + '&url=' + this.LaiKeTuiCommon.LKT_H5_DEFURL
                        }
